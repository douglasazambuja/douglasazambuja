# Oii eu sou Douglas Azambuja, Engenheiro de Dados e apaixonado por tecnologia.

## 🚀 Data Engineer | Databricks Certified Data Engineer Associate

Sou Engenheiro de Dados com experiência no desenvolvimento de pipelines de dados escaláveis, processamento distribuído e arquiteturas Datalake. Tenho atuação em projetos envolvendo ingestão, transformação, modelagem e disponibilização de dados para análises e tomada de decisão.

---

## 🛠️ Tech Stack

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![PySpark](https://img.shields.io/badge/PySpark-E25A1C?style=for-the-badge&logo=apachespark&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=for-the-badge&logo=databricks&logoColor=white)
![Data Lake](https://img.shields.io/badge/Data_Lake-00ADD8?style=for-the-badge)
![Git](https://img.shields.io/badge/Git-F05033?style=for-the-badge&logo=git&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white)

---

## 📚 Áreas de Interesse

- Data Engineering
- Data Platforms
- Data Quality
- Streaming Data
- Lakehouse Architecture
- Analytics Engineering
- Data Governance

---

## 💡 Principais Atividades

- Desenvolvimento de pipelines de ingestão e transformação de dados
- Implementação de processos ETL e ELT
- Processamento distribuído utilizando PySpark
- Modelagem de dados para Analytics
- Automação de deploys e integrações com CI/CD
- Implementação de boas práticas de Engenharia de Dados
- Desenvolvimento de soluções em Databricks e Delta Lake

---

## 🏆 Certificações

- Databricks Certified Data Engineer Associate
- AWS Certified Cloud Practitioner

---

## 📈 Objetivo deste Perfil

Aqui você encontrará projetos relacionados a:

- Engenharia de Dados
- Databricks
- PySpark
- ETL / ELT
- Delta Lake
- Data Quality
- CI/CD
- Arquiteturas Lakehouse

---

⭐ Sempre aprendendo, construindo e compartilhando soluções de dados.
